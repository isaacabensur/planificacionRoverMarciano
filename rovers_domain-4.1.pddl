(define (domain roverwithbattery)
       ...
       ;; Accion para navegar con bateria llena
       (:action navigate_full
              :parameters (?x - rover ?y - waypoint ?z - waypoint)
              :precondition (and (can_traverse ?x ?y ?z) (available ?x) (at ?x ?y) (visible ?y ?z) (battery_full ?x))
              :effect (and (not (at ?x ?y)) (at ?x ?z)
                     (not (battery_full ?x)) (battery_high ?x) (battery_not_full ?x)
              )
       )

       ;; Accion para navegar con bateria alta
       (:action navigate_high
              :parameters (?x - rover ?y - waypoint ?z - waypoint)
              :precondition (and (can_traverse ?x ?y ?z) (available ?x) (at ?x ?y) (visible ?y ?z) (battery_high ?x))
              :effect (and (not (at ?x ?y)) (at ?x ?z)
                     (not (battery_high ?x)) (battery_medium ?x)
              )
       )

       ;; Accion para navegar con bateria media
       (:action navigate_medium
              :parameters (?x - rover ?y - waypoint ?z - waypoint)
              :precondition (and (can_traverse ?x ?y ?z) (available ?x) (at ?x ?y) (visible ?y ?z) (battery_medium ?x))
              :effect (and (not (at ?x ?y)) (at ?x ?z)
                     (not (battery_medium ?x)) (battery_low ?x)
              )
       )

       ;; Accion para navegar con bateria baja
       (:action navigate_low
              :parameters (?x - rover ?y - waypoint ?z - waypoint)
       ...
       (:action recharge
              ...
              :precondition (and (at ?r ?p) (at_lander ?l ?p) (battery_not_full ?r))
              :effect (and (battery_full ?r) (not (battery_high ?r)) (not (battery_medium ?r)) (not (battery_low ?r)) (not (battery_not_full ?r)))
       )
